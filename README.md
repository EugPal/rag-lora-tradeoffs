...
├── README.md
├── requirements.txt
├── pyproject.toml / setup.cfg        # опционально, если хочешь нормальный пакет
├── data/
│   ├── raw/
│   │   ├── fastapi_html/             # сырые HTML-страницы FastAPI
│   │   └── qa_seed/                  # черновики QA (если будут)
│   ├── processed/
│   │   ├── docs.jsonl                # чанки документации
│   │   ├── qa_small.jsonl            # 10–20 QA для прототипа
│   │   ├── qa_train.jsonl
│   │   ├── qa_val.jsonl
│   │   └── qa_test.jsonl
│   └── embeddings/
│       ├── docs_embeddings.faiss     # индекс для поиска
│       └── docs_embeddings.npy       # сами эмбеддинги (если нужно)
├── src/
│   ├── __init__.py
│   ├── config/                       # конфиги экспериментов (yaml)
│   │   ├── base.yaml
│   │   ├── rag_baseline.yaml
│   │   └── lora_L8F.yaml
│   ├── data_pipeline/
│   │   ├── fetch_fastapi_docs.py     # (опционально) скачать доку
│   │   ├── parse_fastapi_docs.py     # распарсить HTML → чистый текст
│   │   ├── chunk_docs.py             # порезать на чанки и собрать docs.jsonl
│   │   └── build_qa_dataset.py       # вспомогалки для QA (сплиты и т.п.)
│   ├── rag/
│   │   ├── embeddings.py             # подсчёт эмбеддингов для чанков/запросов
│   │   ├── index.py                  # создание/загрузка FAISS-индекса
│   │   ├── generator.py              # обёртка над LLM (без LoRA)
│   │   └── rag_pipeline.py           # сборка RAG-пайплайна (retrieval+generation)
│   ├── lora/
│   │   ├── lora_config.py            # утилиты для создания LoRA-конфигов
│   │   ├── train_lora.py             # обучение LoRA (по одному конфигу)
│   │   └── infer_lora.py             # инференс с LoRA-конфигом
│   ├── evaluation/
│   │   ├── metrics.py                # EM/F1, возможно groundedness
│   │   ├── eval_baseline.py          # запуск оценки baseline
│   │   └── eval_lora.py              # запуск оценки LoRA-конфигов
│   └── utils/
│       ├── logging_utils.py          # логирование, таймеры
│       └── io_utils.py               # чтение/запись jsonl, конфигов и т.п.
├── experiments/
│   ├── baseline/
│   │   ├── results.json              # метрики baseline
│   │   └── logs.txt
│   └── lora/
│       ├── L4-S/
│       │   ├── config_used.yaml
...
│       └── L16-F/
└── notebooks/
    ├── 01_explore_docs.ipynb         # посмотреть, как выглядят чанки
    ├── 02_try_rag_baseline.ipynb     # быстрый прототип RAG
    └── 03_plot_results.ipynb         # графики для главы 4
